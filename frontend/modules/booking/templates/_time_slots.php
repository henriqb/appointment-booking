...
<?php foreach ( $slots as $client_timestamp => $slot ) : ?>
    <button <?php disabled( $slot['blocked'], true ) ?>
        data-staff_id="<?php echo esc_attr( $slot['staff_id'] ) ?>"
        data-group="<?php echo esc_attr( $group ) ?>"
        class="ab-available-hour ladda-button<?php if ( $slot['blocked'] ) echo ' booked' ?>"
        ...
        <span class="ladda-label"><i class="ab-hour-icon"><span></span></i>
            <?php echo date_i18n( ( $is_all_day_service ? 'D, M d' : get_option( 'time_format' ) ), $client_timestamp ) ?>
        </span>
    </button>
<?php endforeach ?>